...
function DataUploadForm({ onLanguagesFetched }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (files.length > 0) {
      ...
      try {
        ...
        // Assuming the response will contain the list of languages
        onLanguagesFetched(response.data.languages);
      } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div>
        <input type="file" multiple onChange={e => setFiles(Array.from(e.target.files))} accept=".csv" />
      </div>
